...
contains
  ...
  subroutine check_nonadvective(file_in, vgrid)
    character(len=*), intent(in) :: file_in, vgrid
...
!              write(*,*) i, j, '   North south nonadvective, Deep', num_levels(im, j), num_levels(i, j), num_levels(ip, j)
  ...
  end subroutine check_nonadvective

end module check_nonadvective_m
